...
public class Cdda2wav {
	...
	// CDDA2WAV can not be final because icedax can be present on the system
	private static String CDDA2WAV = CDDA2WAV_COMMAND_STRING;
	...
	public static boolean testTool() {
		...
		try {
			...
			CDDA2WAV = CDDA2WAV_COMMAND_STRING;
			Process p = new ProcessBuilder(CDDA2WAV, VERSION).start();
			stdInput = new BufferedReader(new InputStreamReader(p.getInputStream()));
			while (stdInput.readLine() != null) {
				// On some Linux distributions we have a symlink to icedax so detect it.
				// if (stdInput.readLine().contains("icedax"))
				// CDDA2WAV = "icedax";
			}
			int code = p.waitFor();
			if (code != 0) {
			...
			try {
				CDDA2WAV = ICEDAX_COMMAND_STRING;
				Process icedaxCheck = new ProcessBuilder(CDDA2WAV, VERSION).start();
	...
	public boolean cdda2wav(int track, File fileName) {
		...
		try {
			...
			if (SystemProperties.SYSTEM == OperatingSystem.WINDOWS)
				command.add(StringUtils.getString(Constants.WINDOWS_TOOLS_DIR, SystemProperties.fileSeparator, CDDA2WAV));
			else
	 ...
	 */
	public List<String> doScanBus() {
		logger.info(LogCategories.CDDA2WAV, StringUtils.getString("Scanning bus using ", CDDA2WAV));
		...
		if (CDDA2WAV.equals(ICEDAX_COMMAND_STRING)) {
			BufferedReader stdInput = null;
			try {
				List<String> command = new ArrayList<String>();
				if (SystemProperties.SYSTEM == OperatingSystem.WINDOWS)
		...
		// On some Linux machines -scanbus does not give any CDROM, so lets try it this way:
		if (devices.size() == 0) {
			BufferedReader stdInput = null;
			try {
				List<String> command = new ArrayList<String>();
				...
				while ((s = stdInput.readLine()) != null) {
					if (s.contains("CD-ROM")) {
						...
						if (SystemProperties.SYSTEM == OperatingSystem.SOLARIS) {
							...
							logger.info(LogCategories.CDDA2WAV, "Operating System is " + SystemProperties.SYSTEM.toString() + ", adjusting device path");
							String devPath = null;
				...
				if (cd.getTracks() == 0 || cdLoaded == false) {
					...
					if (getDriveId() != null)
						if (devCounter <= devNumber) {
							...
							logger.info(LogCategories.CDDA2WAV, StringUtils.getString("CDRecursive info: ", cdRecursive));
						}
				}
			} catch (Exception e) {
				logger.error(LogCategories.CDDA2WAV, e);
