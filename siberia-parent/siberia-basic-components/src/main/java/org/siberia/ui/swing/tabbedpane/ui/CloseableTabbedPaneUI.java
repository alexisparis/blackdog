 ...
 */
package org.siberia.ui.swing.tabbedpane.ui;
...
import org.siberia.ui.swing.tabbedpane.CloseableTabbedPane;


/**
 * UI for JTabbedPane who claim for close hability
 * It also provides event to know :
 *          <p> - if a new page has been selected.</p>
 *          <p> - if a new page want to be closed.>/p>
 *          <p> - if a double click has been made on a label of a page.</p>
 * and in the future, il will provides event to :
 *          <p> - warn if a context menu has to be displayed.</p>
 *          <p> - if a selected page want to improve its visibility.</p>
 * 
 * @author alexis
 */
public class CloseableTabbedPaneUI extends BasicTabbedPaneUI
{
    ...
    protected static final int BUTTONSIZE = 10;
    ...
    {   super();
        ...
        /* close button initializations */
        closeImgB = new BufferedImage(BUTTONSIZE, BUTTONSIZE,
                        BufferedImage.TYPE_4BYTE_ABGR);
        closeImgI = 
                new BufferedImage(BUTTONSIZE, BUTTONSIZE,
                        BufferedImage.TYPE_4BYTE_ABGR);
        closeB = new JButton();
        ...
        {    public int	getIconHeight()
             ...
             {   /* set the background color */
                 ...
                 g.drawLine(x + 2, y + 2, x + this.getIconWidth() - 2, y + this.getIconHeight() - 2);
                 g.drawLine(x + 3, y + 2, x + this.getIconWidth() - 2, y + this.getIconHeight() - 3);
                 g.drawLine(x + 2, y + 3, x + this.getIconWidth() - 3, y + this.getIconHeight() - 2);
                 g.drawLine(x + 2, y + this.getIconHeight() - 2, x + this.getIconWidth() - 2, y + 2);
                 g.drawLine(x + 2, y + this.getIconHeight() - 3, x + this.getIconWidth() - 3, y + 2);
                 g.drawLine(x + 3, y + this.getIconHeight() - 2, x + this.getIconWidth() - 2, y + 3);
             }
        };
        
        close.paintIcon(closeB, closeImgI.createGraphics(), 0, 0);
        
//        WindowsIconFactory.createFrameCloseIcon().paintIcon(closeB,
//                        closeImgI.createGraphics(), 0, 0);
        
        this.currentFont = new Font("Arial", Font.PLAIN, 10);
        
        /* initialize the future popup menu on the label of the current page */
        actionPopupMenu = new JPopupMenu();
        closeItem = new JMenuItem("Close");
        closeItem.addActionListener(new ActionListener()
        {   public void actionPerformed(ActionEvent e)
            {   ((CloseableTabbedPane) tabPane).fireCloseTabEvent(null, tabPane.getSelectedIndex());
    ...
    public ScrollableTabPanel getScrollableTabPane()
    ...
    {   if (mouseListener != null)
        ...
        // PENDING(api): See comment for dfsdf
        // PENDING(api): See comment for ContainerHandler
    ...
    {   Rectangle tabRect = rects[tabIndex];
        ...
        /* modify the size of the tab */
        //tabRect.setSize(tabRect.width, (int) ((tabRect.height * 3) / 4));
        ...
        FontMetrics metrics = g.getFontMetrics(this.currentFont);
        Icon icon = getIconForTab(tabIndex);

        layoutLabel(tabPlacement, metrics, tabIndex, title, icon, tabRect,
                        iconRect, textRect, isSelected);
                        
        paintText(g, tabPlacement, this.currentFont, metrics, tabIndex, title, textRect,
                        isSelected);
        ...
        {   int dx = tabRect.x + tabRect.width - BUTTONSIZE - WIDTHDELTA;
            ...
            if (isCloseButtonEnabled)
                paintCloseIcon(g2, dx + 1, dy - 1, isOver);
        }
    ...
    {   Component visibleComponent = getVisibleComponent();
        ...
        else if (visibleComponent instanceof Container)
        {   
//            if (((JComponent) visibleComponent).requestDefaultFocus())
//            {   return true; }
            if ( KeyboardFocusManager.getCurrentKeyboardFocusManager().getDefaultFocusTraversalPolicy().
                     getDefaultComponent((Container)visibleComponent).requestFocusInWindow() )
                return true;
        }
        return false;
    }
    ...
    {   public void actionPerformed(ActionEvent e)
        {   JTabbedPane pane = (JTabbedPane) e.getSource();
            CloseableTabbedPaneUI ui = (CloseableTabbedPaneUI) pane.getUI();
    ...
    {   public void actionPerformed(ActionEvent e)
        {   JTabbedPane pane = (JTabbedPane) e.getSource();
            ...
            int oldSelectedPage = pane.getSelectedIndex();

            if (pane != null && (pane.getUI() instanceof CloseableTabbedPaneUI))
            {   CloseableTabbedPaneUI ui = (CloseableTabbedPaneUI) pane.getUI();
                ...
                {   int mnemonic = (int) e.getActionCommand().charAt(0);
                    ...
                    {   pane.setSelectedIndex(index.intValue());
                        /* tells JTabbedPane that the selection change */
                        ((CloseableTabbedPane)pane).firePageChanged(e, oldSelectedPage, index.intValue());
                    }
                }
            }                       
        }
    ...
    {   public ScrollableTabPanel()
        ...
        {   super.paintComponent(g);
            CloseableTabbedPaneUI.this.paintTabArea(g, tabPane.getTabPlacement(),
    ...
    {   public MyMouseHandler()
        ...
        {   if (!tabPane.isEnabled())
            {   return; }
            int oldSelectedPage = tabPane.getSelectedIndex();
            ...
            {   if (tabIndex == tabPane.getSelectedIndex())
                ...
                {   tabPane.setSelectedIndex(tabIndex);
                    ((CloseableTabbedPane)tabPane).firePageChanged(e, oldSelectedPage, tabIndex);
                }
            }
        ...
        {   super.mousePressed(e);
            int tabToBeClosed = CloseableTabbedPaneUI.this.getTabAtLocation(e.getX(), e.getY());
            if (e.getClickCount() > 1 && overTabIndex != -1)
            {   ((CloseableTabbedPane) tabPane).fireDoubleClickTabEvent(e, overTabIndex); }
        }
        ...
        {   updateOverTab(e.getX(), e.getY());
            ...
            if (overTabIndex == -1)
            {   if (e.isPopupTrigger())
                    ((CloseableTabbedPane) tabPane).firePopupOutsideTabEvent(e);
                return;
            }
            ...
            {   super.mousePressed(e);
                ...
                closeIndexStatus = INACTIVE; //Prevent undesired action when right-clicking on icons
