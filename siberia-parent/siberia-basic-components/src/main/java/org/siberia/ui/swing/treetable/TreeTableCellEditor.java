/*
 * $Id: TreeTableCellEditor.java,v 1.12 2007/02/22 11:45:29 kleopatra Exp $
 *
...
public class TreeTableCellEditor extends DefaultCellEditor {
    ...
    {
	int rowConverted = row;
	
	if ( table instanceof PageableTable )
	{
	    rowConverted = ((PageableTable)table).convertRowIndexToWrapped(row);
	}
	
        Component component = super.getTableCellEditorComponent(table, value,
                isSelected, rowConverted, column);
        // JW: this implementation is not bidi-compliant, need to do better
	
	TreePath path = tree.getPathForRow(rowConverted);
	
	String nodeValue = null;
	
	if ( path != null && path.getLastPathComponent() instanceof SibType )
	{
	    nodeValue = ((SibType)path.getLastPathComponent()).getName();
	}
	
        initEditorOffset(table, path, rowConverted, column, isSelected);
	
	if ( component instanceof JTextComponent )
	{
	    JTextComponent textCmp = (JTextComponent)component;
		...
		{
		    textCmp.setText(nodeValue.toString());
		    textCmp.setSelectionEnd(0);
		    textCmp.setSelectionStart(textCmp.getText().length());
		}
	    }
	}
     ...
     * @param isSelected
     */
    protected void initEditorOffset(JTable table, TreePath path, int rowConverted, int column,
            boolean isSelected) {
        if (tree == null)
	{
            return;
        ...
        if(treeUI != null)
            bounds = treeUI.getPathBounds(tree, path);
	
        int offset = bounds.x;
        Object node = tree.getPathForRow(rowConverted).getLastPathComponent();
        boolean leaf = tree.getModel().isLeaf(node);
        boolean expanded = tree.isExpanded(rowConverted);
        TreeCellRenderer tcr = tree.getCellRenderer();
        Component treeComponent = tcr.getTreeCellRendererComponent(tree, node,
                isSelected, expanded, leaf, rowConverted, false);
