...
public abstract class SibCollection<T extends AbstractCollection<SibType>> extends SibConfigurable
...
//                Iterator<? extends SibType> items = this.iterator();
//                while(items.hasNext())
//                {   SibType current = items.next();
//                    
//                    if ( current != null )
//                    {   this.addChildElement(current); }
//                }
    ...
    {   if ( this.getAllowedClass() != null && item != null )
        {   if ( (this.getAllowedClass()).isAssignableFrom(item.getClass()) )
                return true;
        }
        return false;
    }
    ...
    {   if ( c != null )
        {   if ( this.collection == null )
            ...
            {   /* fire event */
                ...
                int index = 0;
                for(Iterator<SibType> it2 = items.iterator(); it2.hasNext();)
	...
	if ( this.collection != null )
        {   
	    List<SibType> copy = new ArrayList<SibType>(this.collection);
	    
	    this.collection.clear();
	    
	    if ( this.isContentItemAsChild() )
	    {
		this.removeChildElements(copy);
	    }
            
            /* tell that the collection has been cleared */
            this.firePropertyChange(new ContentClearedEvent(this, PROPERTY_CONTENT, (SibType[])copy.toArray(new SibType[copy.size()]) ));
        }
    }
