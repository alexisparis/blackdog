...
{   
    ...
    {   if ( this.relatedClass == null )
            throw new UnsupportedOperationException("related class is not initialized");
    ...
    {   this.automaticBeanDescriptor = automaticBeanDescriptor;
        ...
        /** free the BeanDescriptor ... */
        if ( automaticBeanDescriptor && this.descriptor != null )
    ...
    {   BeanDescriptor descriptor = new BeanDescriptor(this.getClass());
        ...
        if ( type != null )
        {   descriptor.setDisplayName(TypeInformationProvider.getDisplayName(this.getRelatedClass()));
            descriptor.setName(type.name());
            descriptor.setExpert(type.expert());
            descriptor.setHidden(type.hidden());
            descriptor.setPreferred(type.preferred());
            descriptor.setShortDescription(TypeInformationProvider.getShortDescription(this.getRelatedClass()));
        }
        else
        {   descriptor.setName("unknown");
    ...
    {   this.automaticPropertiesDescriptor = automaticPropertiesDescriptor;
        ...
        /** free the PropertyDescriptor array ... */
        if ( automaticPropertiesDescriptor && this.properties != null )
        ...
        {   if ( this.properties == null )
            {   
                ...
                {   Class current    = this.getRelatedClass();
                    ...
                    /* createPropertiesDescriptor already care about finding PropertyDescriptors
                     *  for superclass of current
                     */
//                    while(current != null && limitClass.isAssignableFrom(current))
//                    {   List<PropertyDescriptor> currentList = this.createPropertiesDescriptor(current);
//                        logger.debug("getting " + currentList.size() + " properties descriptor for class " + current);
//                        
//                        if ( list == null )
//                        {   if ( currentList != Collections.EMPTY_LIST )
//                                list = currentList;
//                            // else, nothing to do, it is empty
//                        }
//                        else
...
//                        
//                        logger.debug("current in loop : " + current);
//                    }
			...
			try
			{   
			    ExtendedPropertyDescriptor identityHashCodeDescriptor = new ExtendedPropertyDescriptor(SibType.PROPERTY_IDENTITY_HASHCODE,
									       current.getMethod("getIdentityHashCode", (Class[])null),
									       null);
			    identityHashCodeDescriptor.setSupportGrouping(true);
			...
			catch (IntrospectionException ex)
			{
                ...
                {   /* consider that we are no PropertyDescriptor */
                    this.properties = new PropertyDescriptor[]{};
                }
            }
    ...
    {   List<PropertyDescriptor> propertiesList = null;
        ...
        if ( propertiesList == null )
            propertiesList = Collections.EMPTY_LIST;
        ...
        {   if ( this.shouldGenerate(field) )
            {   
                ...
                {   String name = property.name();
                    ...
                    if ( name != null )
                    {   
                        /** if the name is not in the list of allowed propertes name, then
                         *  abort
                         */
                        boolean generate = true;
			
			/** if category is BeanInfoCategory.CONFIGURATION or BeanInfoCategory.ALL
			 *  then do not take allowed properties into account
			 */
                        ...
                        {   try
                            {   Method readMethod  = this.getReadMethod(field, property);
                                ...
                                ExtendedPropertyDescriptor extendedDescriptor = new ExtendedPropertyDescriptor(name, readMethod, writeMethod);//this.relatedClass);
				
				UnmergeableProperty unmergeableAnnotation = (UnmergeableProperty)field.getAnnotation(UnmergeableProperty.class);
				
				if ( unmergeableAnnotation != null )
				{
				    extendedDescriptor.setSupportGrouping(false);
				}
				
				descriptor = extendedDescriptor;

                                /** offers some opportunities to modify the descriptor before building */
                                ...
                                /** offers some opportunities to modify the descriptor before completed */
    ...
    {   this.automaticMethodsDescriptor = automaticMethodsDescriptor;
        ...
        /** free the MethodDescriptor array ... */
        if ( automaticMethodsDescriptor && this.methods != null )
            this.methods = null;
    }
...
//        return meths;
    ...
    {   List<MethodDescriptor> methodList = null;
        ...
        if ( methodList == null )
            methodList = Collections.EMPTY_LIST;
    ...
    {   MethodDescriptor descriptor = null;
        ...
        {   if ( this.shouldGenerate(method) )
            {   
                ...
                {   
//                    descriptor = new MethodDescriptor(null);
    ...
    {   Image img = null;
        ...
        if ( this.isPluginContextActivated() )
        {
            ...
            {   
                switch(iconKind)
                {   case BeanInfo.ICON_MONO_16x16  : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_PLUGIN_ICON_MONO_16);
                                                     break;
                    case BeanInfo.ICON_MONO_32x32  : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_PLUGIN_ICON_MONO_32);
                                                     break;
                    case BeanInfo.ICON_COLOR_16x16 : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_PLUGIN_ICON_COLOR_16);
                                                     break;
                    case BeanInfo.ICON_COLOR_32x32 : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_PLUGIN_ICON_COLOR_32);
                                                     break;
                }
            }
        }
        else
        {
            ...
            {   
                switch(iconKind)
                {   case BeanInfo.ICON_MONO_16x16  : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_ICON_MONO_16);
                                                     break;
                    case BeanInfo.ICON_MONO_32x32  : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_ICON_MONO_32);
                                                     break;
                    case BeanInfo.ICON_COLOR_16x16 : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_ICON_COLOR_16);
                                                     break;
                    case BeanInfo.ICON_COLOR_32x32 : rcPath = TypeInformationProvider.getIconInformation(this.getRelatedClass(), BeanConstants.BEAN_ICON_COLOR_32);
                                                     break;
                }
            }
        }
